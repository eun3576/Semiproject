						...
						<table class="table table-border table-hover table-striped" style="text-align: center;">
							...
							<%for(int i=0;  i < userList.size(); i++){ %>
								<tr>
									...
									<td><span id="notice-header-type"><%=userList.get(i).getId() %></span></td>
									<td style="text-align: left;"><%=userList.get(i).getPassword() %></td>
									<td><%=userList.get(i).getNickname() %></td>
									<td><%=userList.get(i).getGender() %></td>
									<td><%=userList.get(i).getSymptom() %></td>
									<td><%=userList.get(i).getPhonenumber() %></td>
									<td><a class="update-btn" style="background-color: #FF5A5A;" href="<%= root%>/manager/userDelete?user_no=<%= userList.get(i).getUserNo()%>">삭제</a></td>
								</tr>
							<%} %>
							</tbody>
						</table>
