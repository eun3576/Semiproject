...
function chatClose(){
// 	console.log("ajaxGetServerList() called")
	if(confirm("채팅방 내용이 삭제됩니다")==true){
	//전달 파라미터 저장
	var params = "chatroom="+chatroom.value;
	console.log(params)
	
	sendRequest("POST", "/chat/user", params, ajaxDeleteServer);
	}else{
		return false;
	}
}

function ajaxDeleteServer(){
	console.log("ajaxDeleteServer() called")
	if(httpRequest.readyState==4){//done, 응답완료
		if(httpRequest.status==200){//정상응답
			console.log("ajaxDeleteServer() 정상응답")
			window.close();
		}else{
			console.log("삭제 요청 오류")
		}
	}
}
...
<button class="btn btn-danger" style="width:70px;height:30px;float: right;margin-top:10px;margin-right:10px;" onclick="chatClose();">나가기</button>
</div>
...
<input type="text" value="<%=userid %>" style="display:none;" name="chatId" id="chatId">
<input type="text" value="<%=userid %>" style="display:none;" name="chatroom" id="chatroom">
